...
for (i in 1:693) {
	...
	data[[i]] <- max[data[[i]], on = .(stock.id, year, sem), nomatch = 0
	][!is.na(max)
	][, stock.p := MarketValue / sum(MarketValue, na.rm = TRUE), keyby = .(date)
	][, .(MAX = sum(stock.p * max)), keyby = .(date)
	][, id := fund1]
...
syf <- syf[, year := year(date)
	...
	][, .(id, year, month, date, MAX)]


save(syf, file = "fund-MAX.RData")
...
data <- data[order(id, date)
	...
	][, max.g := ntile(MAX, 5), keyby = .(date)]

TK.ret <- data[!is.na(month_return.1)
	][, .(ret = mean(month_return.1)), keyby = .(date, max.g)
	][!is.na(ret)]
...
data <- data[, date := as.Date(date)
	...
	][, max.g := ntile(MAX, 5), keyby = .(date, month_return.g)]

TK.g <- data[!is.na(month_return.1)
	][, .(ret = mean(month_return.1)), keyby = .(date, max.g, month_return.g)
	][!is.na(ret)]

low <- TK.g[max.g == 1 & month_return.g == 1]
high <- TK.g[max.g == 5 & month_return.g == 1]
dif <- low[high, on = .(date)
	][, dif := i.ret - ret]

t.test(TK.g[max.g == 5 & month_return.g == 1, ret])
t.test(dif[, dif])
...
## reg
reg <- data[, felm(month_return.1 ~ MAX  | id + year)] %>% summary()

stargazer(reg, type = "html", out = "C:/Users/shenfan/Desktop/prospect value/tables/max and performance.doc", add.lines = list(c("Fund", "yes"), c("year", "yes")))
