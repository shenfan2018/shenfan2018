...
fund.TK <- fund.TK[, quarter := quarter(date)]

# 由monthly变成quarterly
fund.TK.q <- fund.TK[, month_return2 := month_return + 1
	][, .(quarter_return = prod(month_return2) - 1, TK = mean(TK), PW = mean(PW), LA = mean(LA), CC = mean(CC)), keyby = .(id, year, quarter)]

fund.TK48.q <- fund.TK[, month_return2 := month_return + 1
	][, .(quarter_return = prod(month_return2) - 1, TK48 = mean(TK48), PW48 = mean(PW48), LA48 = mean(LA48), CC48 = mean(CC48)), keyby = .(id, year, quarter)]

fund.TK36.q <- fund.TK[, month_return2 := month_return + 1
	][, .(quarter_return = prod(month_return2) - 1, TK36 = mean(TK36), PW36 = mean(PW36), LA48 = mean(LA48), CC48 = mean(CC48)), keyby = .(id, year, quarter)]
...
data <- fundflow[fund.TK.q, on = .(id, year, quarter), nomatch = 0]
...
TK.g <- data2[, netflow.g := ntile(netflow.1, 5), keyby = .(year, quarter)
	][, .(TK.c = mean(LA.c)), keyby = .(year, quarter, netflow.g)]
