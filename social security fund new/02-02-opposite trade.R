...
# 公募基金持仓变化
load("fundpf.RData")
fundpf <- fundpf[, .(id, date, stock.id, current_hold, hold_change, current_mv)]
setnames(fundpf, "hold_change", "fund_change")
...
# add fund_size
fund_size <- fundpf[, .(fund_size = sum(current_mv), fund_hold = (.N)), keyby = .(id, date)]
