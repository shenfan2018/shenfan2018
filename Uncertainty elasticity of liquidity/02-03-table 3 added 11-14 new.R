...
# match national
IO <- IO.national[IO, on = .(stock.id, date)
	][, national := ifelse(is.na(national), 0, national)]

## add
IO <- IO[, lapply(colnames(IO[, 3:8]), str_c, ".shift") %>% unlist() := lapply(.SD[, 2:7], shift, n = 1, fill = NA, type = 'lag'), keyby = .(stock.id)
	...
	][, national.buy := ifelse(national > national.shift, 1, 0)]

IO <- IO[, year := year(date)
	][, quarter := quarter(date)
	][, .(stock.id, date, year, quarter, national.buy, IOS.buy, IOF.buy, IOI.buy, IOQ.buy, sum.buy)]

# 仅选择2015.2到2017.2
IO <- IO[date > as.Date("2015-03-31") & date < as.Date("2017-09-30")]
...
####################################
# new choose的有问题
table <- table[, choose := shift(value, n = 1, fill = NA, type = 'lead'), keyby = .(variable, stock.id)
	][!is.na(choose) & !is.na(uel.m) & !is.na(uel.dif)]
	#	][order(variable, year, quarter, uel.m)
	#][, vartag := ifelse(variable == 'IOS.buy', 1, ifelse(variable == 'IOF.buy', 2, ifelse(variable == 'IOI.buy', 3, ifelse(variable == 'IOQ.buy', 4, ifelse(variable == "IOnational.buy", 5, 6)))))
...
# no PSM
a <- table[year == 2015 & quarter == 2
	][, .(uel.m = mean(uel.m), uel.m.2 = mean(uel.m.2), t = coeftest(lm(uel.dif ~ 1), vcov. = NeweyWest)[, 3], dif = t.test(uel.dif)$estimate, t2 = t.test(uel.dif)$statistic), keyby = .(year, quarter, choose, variable)]

write.csv(a, "C://Users//shenfan//Desktop//mydatam.csv")

# DIF
a <- syf[order(variable, year, quarter, choose, uel.m)
	][, rank := 1:.N, keyby = .(tag, choose)]
choose1 <- a[choose == 1, .(variable, stock.id, year, quarter, uel.dif, rank, choose)]
choose2 <- a[choose == 0, .(variable, stock.id, year, quarter, uel.dif, rank, choose)]
choose <- choose1[choose2, on = .(variable, year, quarter, rank)
	][, dif := uel.dif - i.uel.dif
	][, .(dif = mean(dif)), keyby = .(variable, year, quarter)
	][, .(dif = t.test(dif)$estimate, t = coeftest(lm(dif ~ 1), vcov. = NeweyWest)[, 3], t2 = t.test(dif)$statistic, p = coeftest(lm(dif ~ 1), vcov. = NeweyWest)[, 4]), keyby = .(variable)]

write.csv(choose, "C://Users//shenfan//Desktop//mydatam.csv")

############## PSM
table <- table[, choose := shift(value, n = 1, fill = NA, type = 'lead'), keyby = .(variable, stock.id)
	][!is.na(choose) & !is.na(uel.m) & !is.na(uel.dif)
	][order(variable, year, quarter, uel.m)
	][, vartag := ifelse(variable == 'IOS.buy', 1, ifelse(variable == 'IOF.buy', 2, ifelse(variable == 'IOI.buy', 3, ifelse(variable == 'IOQ.buy', 4, ifelse(variable == "national.buy", 5, 6)))))
	][, tag := (vartag - 1) * 100 + (year - 2004) * 5 + quarter]
...
#a <- syf[order(variable, year, quarter, uel.m)
...
# buy and not buy
a <- syf[year == 2015 & quarter == 2
	][, .(uel.m = mean(uel.m), uel.m.2 = mean(uel.m.2), dif = t.test(uel.dif)$estimate, t = coeftest(lm(uel.dif ~ 1), vcov. = NeweyWest)[, 3], t2 = t.test(uel.dif)$statistic), keyby = .(year, quarter, choose, variable)]

write.csv(a, "C://Users//shenfan//Desktop//mydatam.csv")

# DID
a <- syf[year == 2015 & quarter == 2
	][order(variable, year, quarter, choose, uel.m)
	][, rank := 1:.N, keyby = .(tag, choose)]
choose1 <- a[choose == 1, .(variable, stock.id, year, quarter, uel.dif, rank, choose)]
choose2 <- a[choose == 0, .(variable, stock.id, year, quarter, uel.dif, rank, choose)]
choose <- choose1[choose2, on = .(variable, year, quarter, rank)
...
IO <- IO[, year := year(date)
	...
	][, .(stock.id, date, year, quarter, IOS.buy, IOF.buy, IOI.buy, IOQ.buy, sum.buy)]

# match
data <- uel.m[IO, on = .(stock.id, year, quarter), nomatch = 0]

table = melt(data, id.vars = 1:6, measure.vars = 8:12)
...
# buy and not buy
a <- syf[, .(uel.m = mean(uel.m), uel.m.2 = mean(uel.m.2), uel.dif = mean(uel.dif)), keyby = .(variable, year, quarter, choose)
	][, .(uel.m = mean(uel.m), uel.m.2 = mean(uel.m.2), dif = t.test(uel.dif)$estimate, t = coeftest(lm(uel.dif ~ 1), vcov. = NeweyWest)[, 3], t2 = t.test(uel.dif)$statistic), keyby = .(choose, variable)]
