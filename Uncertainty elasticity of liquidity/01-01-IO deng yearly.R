...
IO <- IO[!is.na(name)
	...
	][, .(IO = mean(IO)), keyby = .(stock.id, year)]
...
IOS <- IOS[!is.na(name)
	...
	][, .(IOS = mean(IOS)), keyby = .(stock.id, year)]
# match
IO <- IOS[IO, on = .(stock.id, year)]
...
IOF <- IOF[!is.na(name)
	...
	][, .(IOF = mean(IOF)), keyby = .(stock.id, year)]
# match
IO <- IOF[IO, on = .(stock.id, year)]
...
IOQ <- IOQ[!is.na(name)
	...
	][, .(IOQ = mean(IOQ)), keyby = .(stock.id, year)]
# match
IO <- IOQ[IO, on = .(stock.id, year)]
...
IOI <- IOI[!is.na(name)
	...
	][, .(IOI = mean(IOI)), keyby = .(stock.id, year)]
# match
IO <- IOI[IO, on = .(stock.id, year)]
...
IOSE <- IOSE[!is.na(name)
	...
	][, .(IOSE = mean(IOSE)), keyby = .(stock.id, year)]
# match
IO <- IOSE[IO, on = .(stock.id, year)]
...
IOT <- IOT[!is.na(name)
	...
	][, .(IOT = mean(IOT)), keyby = .(stock.id, year)]
# match
IO <- IOT[IO, on = .(stock.id, year)]
...
IOM <- IOM[!is.na(name)
	...
	][, .(IOM = mean(IOM)), keyby = .(stock.id, year)]
# match
IO <- IOM[IO, on = .(stock.id, year)]
...
IOB <- IOB[!is.na(name)
	...
	][, .(IOB = mean(IOB)), keyby = .(stock.id, year)]
# match
IO <- IOB[IO, on = .(stock.id, year)]
...
IOC <- IOC[!is.na(name)
	...
	][, .(IOC = mean(IOC)), keyby = .(stock.id, year)]
# match
IO <- IOC[IO, on = .(stock.id, year)]
...
IOP <- IOP[!is.na(name)
	...
	][, .(IOP = mean(IOP)), keyby = .(stock.id, year)]
# match
IO <- IOP[IO, on = .(stock.id, year)]
...
IONF <- IONF[!is.na(name)
	...
	][, .(IONF = mean(IONF)), keyby = .(stock.id, year)]
# match
IO <- IONF[IO, on = .(stock.id, year)]
...
write.csv(IO, "C://Users//shenfan//Desktop//mydatam.csv")
