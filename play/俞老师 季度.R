...
# 筛选top20的基金 (大概是top20%的基金，他们的持仓都没有显示，特别在2011）
a <- Fund.s[a, on = .(id, year, sem), nomatch = 0
	][order(year, sem)
	][, E_ntile := ifelse(top20 == 5, ntile(E_score, 5), 0), keyby = .(year, sem)
	][, Q_ntile := ifelse(top20 == 5, ntile(Q_score, 5), 0), keyby = .(year, sem)
#	][top20==5, E_ntile := ntile(E_score, 5), keyby = .(year, sem)
#	][top20==5, Q_ntile := ntile(Q_score, 5), keyby = .(year, sem)
	][year < 2017]

a <- a[order(id, year, sem)]

a <- a[, Et0 := ifelse(top20 < 5, -1, ifelse(E_ntile == 5, 1, 0))
	][, Qt0 := ifelse(top20 < 5, -1, ifelse(Q_ntile == 5, 1, 0))
	][, Dt0 := ifelse(top20 < 5, -1, ifelse(E_ntile == 5 | Q_ntile == 5, 0, 1))] ## 改
...
Fund.R2 <- a[Fund.m, on = .(id, year, sem)
	][order(id, year, month)]
...
SJCS <- Fund.R2[, .(id, year, quarter, Et0)
	...
	][, Et1 := shift(Et0, n = 1, fill = NA, type = "lag"), keyby = .(id)
	][quarter == 2 | quarter == 4, Et1 := 0
	][, Et2 := shift(Et1, n = 1, fill = NA, type = "lag"), keyby = .(id)
	][, Et3 := shift(Et2, n = 1, fill = NA, type = "lag"), keyby = .(id)
	][, Et4 := shift(Et3, n = 1, fill = NA, type = "lag"), keyby = .(id)
	][, Et5 := shift(Et4, n = 1, fill = NA, type = "lag"), keyby = .(id)
	][, Et6 := shift(Et5, n = 1, fill = NA, type = "lag"), keyby = .(id)]
...
SJCS <- Fund.R2[, .(id, year, quarter, Qt0)
	...
	][, Qt1 := shift(Qt0, n = 1, fill = NA, type = "lag"), keyby = .(id)
	][quarter == 2 | quarter == 4, Qt1 := 0
	][, Qt2 := shift(Qt1, n = 1, fill = NA, type = "lag"), keyby = .(id)
	][, Qt3 := shift(Qt2, n = 1, fill = NA, type = "lag"), keyby = .(id)
	][, Qt4 := shift(Qt3, n = 1, fill = NA, type = "lag"), keyby = .(id)
	][, Qt5 := shift(Qt4, n = 1, fill = NA, type = "lag"), keyby = .(id)
	][, Qt6 := shift(Qt5, n = 1, fill = NA, type = "lag"), keyby = .(id)]
...
SJCS <- Fund.R2[, .(id, year, quarter, Dt0)
	...
	][, Dt1 := shift(Dt0, n = 1, fill = NA, type = "lag"), keyby = .(id)
	][quarter == 2 | quarter == 4, Dt1 := 0
	][, Dt2 := shift(Dt1, n = 1, fill = NA, type = "lag"), keyby = .(id)
	][, Dt3 := shift(Dt2, n = 1, fill = NA, type = "lag"), keyby = .(id)
	][, Dt4 := shift(Dt3, n = 1, fill = NA, type = "lag"), keyby = .(id)
	][, Dt5 := shift(Dt4, n = 1, fill = NA, type = "lag"), keyby = .(id)
	][, Dt6 := shift(Dt5, n = 1, fill = NA, type = "lag"), keyby = .(id)]
