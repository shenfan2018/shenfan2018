...
best <- Fund.s[portfolio, on = .(id, year, sem), nomatch = 0]
...
best <- MA[best, on = .(stock.id, year), nomatch = 0
...
# 赋予ETD
best <- best[order(year, sem)
	][, E_ntile :=  ntile(E_score, 5), keyby = .(year, sem)
	][, Q_ntile := ntile(Q_score, 5), keyby = .(year, sem)
	][year < 2017
	][, Et0 := ifelse(E_ntile == 5, 1, 0)
	][, Qt0 := ifelse(Q_ntile == 5, 1, 0)
	][, Dt0 := ifelse(E_ntile == 5 | Q_ntile == 5, 0, 1)
	][, .(id, year, sem, Et0, Qt0, Dt0)]
...
# 匹配上
Fund.R2 <- best[Fund.m, on = .(id, year, sem)]
...
ggplot(syf, aes(x = period)) +
	geom_line(aes(y = alpha_4, linetype = group))
