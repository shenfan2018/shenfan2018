...
data <- SJ
data <- data[order(year, month, category2, - month_return.1)
	][, rank.1 := sequence(.N), keyby = .(year, month, category2)]

# 这里啊 要把NA的去掉哦(这里还是考虑到了的）
data <- data[!is.na(month_return.1)
	][, per.tb := ntile(rank.1, 5), keyby = .(year, month)]
...
t.test(data[per.tb == 5, "ztk"])

t.test(data[flow.tb == 1, "ztk"])

t.test(data[per.tb == 1 & flow.tb == 2, "ztk"])

t.test(data[per.tb == 4 & flow.tb == 5, "ztk"], data[per.tb == 4 & flow.tb == 1, "ztk"], paired = FALSE)

t.test(data[, "ztk"])

t.test(data[per.tb == 1, "ztk"], data[per.tb == 5, "ztk"], paired = FALSE)

t.test(data[flow.tb == 5, "ztk"], data[flow.tb == 1, "ztk"], paired = FALSE)
...
data <- date[data, on = .(year, month), nomatch = 0
	]

liner <- plm(ztk ~ netflow.1 + rank.1, data, model = "within", effect = "twoways", index = c("id", "date"))

stargazer(liner, type = "html", out = "C:/Users/shenfan/Desktop/prospect value/tables/zhihou1.doc", add.lines = list(c("Fund", "yes"), c("date", "yes")))


##
a <- data[order(year, sem, category2, - sem_return)
	][, rank := sequence(.N), by = .(year, month, category2)]

plm(ztk ~ netflow.1 + rank.1, a, model = "within", effect = "twoways", index = c("id", "date")) %>% summary()
